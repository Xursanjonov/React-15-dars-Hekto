...
export const ProductDetails = () => {
    const { id } = useParams();
    const date = data?.find((item) => item.id == id)
    console.log(date);

    return (
        <>
            <section className="container">
                <div id={date.id} className="carousel-item w-[19rem] card mx-auto bg-gray-100 text-black hover:bg-blue-500 hover:text-white">
                    <figure className="object-contain w-full h-[15rem] bg-white p-[3rem]">
                        <img className="bg-white" src={date.url} alt="Shoes" />
                    </figure>
